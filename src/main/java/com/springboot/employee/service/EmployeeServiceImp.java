...
public class EmployeeServiceImp implements EmployeeService {
	...
	public List<Employee> getAllEmployee() {
		...
		employeeEsService.getAllEsEmployee();
	...
	public Employee getEmployee(Integer id) {
		...
		if (e.isPresent()) {
			...
			employeeEsService.getEsEmployee(id);
			return destionation;
		} else {
			return null;
		}
	...
	public Employee addEmployee(Employee employee) {
		...
		savedEmployee.setAddress(savedAddress); //To make it bidirectional
		System.out.println("add employee ->" + savedEmployee);

		// employeeEsService.addEsEmployee(savedEmployee);
		// addressEsService.addAddress(savedAddress);
		// employeeEsRepository.save(Employeedocument);
		// List<FamilyInfo> info = familyInfoService.addInfo(f);
		// e.setFamilyInfo(info);
		// TODO: Published employeeCreated application event
		LOGGER.info("publish add event of employee  started :");
		EmployeeCreatedEvent employeeCreatedEvent = new EmployeeCreatedEvent(savedEmployee.getEmployId());
		applicationEventPublisher.publishEvent(employeeCreatedEvent);
	...
	public void deleteEmployee(Integer id) {
		...
		if (e.isPresent()) {
			...
			addressEsService.removeAddress(emp.getAddress());
			employeeRepository.deleteById(id);
			// employeeEsService.deleteEsEmployee(id);
			// TODO: Published employeDeleted application event
			LOGGER.info("employee deleted evented start...");
			EmployeeDeletedEvent employeeDeletedEvent = new EmployeeDeletedEvent(id);
			applicationEventPublisher.publishEvent(employeeDeletedEvent);
		} else {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Employee not found with this id");
		}
	}
	...
	private Address manageAdderess(Address requestAddress, Address dbAddress, Employee emp) {
		if (requestAddress != null && dbAddress != null) {
			...
			// addressEsService.updateAddress(requestAddress);
	...
	public Employee updateEmployee(Employee employee) {
		...
		if (!e.isPresent()) {
			...
			addressEsService.updateAddress(add);
			// employeeEsService.updateEmployee(emp);
			entityManager.flush(); // forcefully add to database, Synchronize the persistence context to the
									// underlying database.

			// TODO: Published employeeUpdated application event
			EmployeeUpdatedEvent employeeUpdatedEvent = new EmployeeUpdatedEvent(e1.getEmployId());
...
//			}
			...
			try {
				LOGGER.info("Starting run propertyUtils..");
				PropertyUtils.setProperty(dbEmployee, entry.getKey(), entry.getValue());
			} catch (IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
				e.printStackTrace();
			}
		});
		LOGGER.info("save dbEmployee");
		Employee updatedEmployee = employeeRepository.save(dbEmployee);
		// TODO: Published employeeUpdated application event
		EmployeeUpdatedEvent employeeUpdatedEvent = new EmployeeUpdatedEvent(Integer.parseInt(id));
		applicationEventPublisher.publishEvent(employeeUpdatedEvent);
		LOGGER.info("return dozerbeanMapper");
